...
class Solution:
    def equalPairs(self, grid: list[list[int]]) -> int:
        ...
        counter = 0
        for row in setColumns:
            if row in setRows: #If we find a row/column that is present both as a row and a column in the matrix...
                counter += setColumns[row] * setRows[row] #Since we are looking for unique row pair INDEXES, it is okay if the values within each matching row/column are duplicate
