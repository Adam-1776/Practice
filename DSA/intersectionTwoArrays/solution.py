...
class Solution:
    ...
    def intersectII_2(self, nums1: list[int], nums2: list[int]) -> list[int]:
        if len(nums1) > len(nums2) : #Expect nums1 to be smaller than nums2
        ...
        for num in nums2:
            if num in dict1: #If a num is present in both nums1 and nums2...
                dict1[num] -= 1 #Decrement the count of this num in the dictionary
                if dict1[num] == 0 : dict1.pop(num) #Remove this num from the dictionary if its count is now zero
    ...
    def intersectII_3(self, nums1: list[int], nums2: list[int]) -> list[int]:
        ...
        while i < len(nums1) and j < len(nums2):
            if nums1[i] < nums2[j]: #We want to get both pointers to have the same value
                ...
                j += 1
            else: #If both pointers are pointing to indexes with the same value
